...
```bash
test-multistep$ sam logs -n StockCheckerFunction --stack-name test-multistep --tail
```
...
```bash
test-multistep$ cd functions/add-member-to-dynamo
add-member-to-dynamo$ npm install
add-member-to-dynamo$ npm run test
```
...
```bash
aws cloudformation delete-stack --stack-name test-multistep
```
